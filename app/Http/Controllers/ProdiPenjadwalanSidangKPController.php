...
class ProdiPenjadwalanSidangKPController extends MasterController
{
    ...
    public function index() {
        $breadcrumbs = $this->getBreadCrumbs('penjadwalanSidangKP');
        $title = $this->getTitle('penjadwalanSidangKP');
        $sub_title = $this->getSubTitle('penjadwalanSidangKP');
        $penjadwalan_css = 'active';
        $jadwalKP_css = 'active';
        return view('prodi.schedule.kp', compact('breadcrumbs', 'title', 'sub_title', 'penjadwalan_css', 'jadwalKP_css'));
    }

    /**
     * datatables get list of penjadwalan kp
     */
    public function getListOfPenjadwalanKP(Request $request) {
        $requests = Jadwal::join('requests AS req', 'penjadwalan_sidang.request_id', '=', 'req.id')
                    ...
                    ->where('req.type', CreationType::KP)
    ...
    public function getPenjadwalanForStoreData(Jadwal $penjadwalan) {
        ...
        $breadcrumbs = $this->getBreadCrumbs('formulirPenjadwalanSidangKP');
        $title = $this->getTitle('formulirPenjadwalanSidangKP');
        $sub_title = $this->getSubTitle('formulirPenjadwalanSidangKP');
        $penjadwalan_css = 'active';
        $jadwalKP_css = 'active';
        // set pembimbing value, penguji value, pembimbing back up value
        // $pembimbingValue = $prodiUserModel::where('id', $penjadwalan->dosen_pembimbing_id)->first()->username;
        ...
        return view('prodi.schedule.kp_schedule', compact('breadcrumbs', 'title', 'sub_title', 'penjadwalan_css', 
        'jadwalKP_css', 'penjadwalan', 'listOfProdiDDL', 'pembimbingValue', 'isRevision'));
    }
...
// dd('ha');
    ...
    public function notifyProdiAdminToRescheduleIfNeed() {
        ...
        $penjadwalanSidang = $penjadwalanSidangModel::where('status_penjadwalan', 'COMPLETED')
                            ...
                            ->where('sidang_type', CreationType::KP)
                            ->get();
        ...
        foreach($penjadwalanSidang as $object) {            
            try {
                ...
                Log::info($e);
                Log::info('gagal update status penjadwalan '. CreationType::KP .' untuk revisi ' . Carbon::today(). ' pada id '. $object->id);
            }
        ...
        if($needRevise) {
            ...
            $penjadwalanSidangDistinct = $penjadwalanSidangModel::distinct()->where('sidang_type', CreationType::KP)->get(['penjadwalan_by']);
            foreach($penjadwalanSidangDistinct as $object) {
                ...
                try {
                    $kp = 'Kerja Praktek';
                    Mail::to($prodiAdmin)->send(new ReschedulePenjadwalanNotification($prodiName, $kp));
    ...
    public function getRoute($key, $id = null) {
        switch($key) {
            case 'list':
                return route('prodi.penjadwalan.kp');
            case 'view':
                return route('prodi.penjadwalan.view.kp', $id);
            case 'jadwal':
                return route('prodi.penjadwalan.assign.kp', $id);
            default:
                break;
        }
    }
    ...
    public function viewPenjadwalan(Jadwal $penjadwalan) {
        $breadcrumbs = $this->getBreadCrumbs('lihatPenjadwalanSidangKP');
        $title = $this->getTitle('lihatPenjadwalanSidangKP');
        $sub_title = $this->getSubTitle('lihatPenjadwalanSidangKP');
        $penjadwalan_css = 'active';
        $jadwalKP_css = 'active';
        ...
        $backRoute = route('prodi.penjadwalan.kp');

        return view('admin.schedule.view_kp_schedule', 
                compact('breadcrumbs', 'title', 'sub_title', 'penjadwalan_css', 'jadwalKP_css', 'backRoute',
