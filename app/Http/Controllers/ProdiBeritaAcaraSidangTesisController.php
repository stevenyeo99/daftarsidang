...
class ProdiBeritaAcaraSidangTesisController extends MasterController {
    ...
    public function index() {
        ...
        return view('prodi.berita_acara.tesis.tesis', compact('breadcrumbs', 'title', 'sub_title', 'beritaAcaraMaster_css', 'beritaAcaraTesis_css', 'ruanganSidangArr'));
    }
    ...
    public function getRoute($key, $id = null) {
        switch($key) {
            case 'list':
                return route('prodi.berita.acara.tesis');
            case 'form':
                return route('prodi.berita_acara_form.tesis', $id);
            case 'save':
                return '';
            default:
                break;
    ...
    public function formSidang(Penguji $participant) {
        ...
        $breadcrumbs = $this->getBreadCrumbs('formulirBeritaAcaraSidangTesis');
        $title = $this->getTitle('formulirBeritaAcaraSidangTesis');
        $sub_title = $this->getSubTitle('formulirBeritaAcaraSidangTesis');
        $beritaAcaraMaster_css = 'active';
        $beritaAcaraTesis_css = 'active';
        $beritaAcaraMaster = $participant->report()->first();
        $listOfNoteRevision = $participant->revisi()->get();

        return view('prodi.berita_acara.tesis.tesis_form', compact('beritaAcaraMaster', 'participant', 'listOfNoteRevision',
         'breadcrumbs', 'title', 'sub_title', 'beritaAcaraMaster_css', 'beritaAcaraTesis_css'));
    }
        ...
        try {
            ...
            if($participantType == 1) {
                ...
                if(count($arrayNotes) > 0) {
                    Mail::to($studentEmaillAddress)->send(new NoteRevisiSidangMahasiswa($acara, $beritaAcaraParticipant, $arrayNotes, 'Tesis'));
                }
                ...
                Mail::to($studentEmaillAddress)->send(new HasilSidangMahasiswa($acara, 'Tesis', $isLulus));
            } else {
