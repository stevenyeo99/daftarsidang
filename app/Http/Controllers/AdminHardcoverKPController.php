...
class AdminHardcoverKPController extends MasterController
{
    ...
    public function index() {
        ...
        return view('admin.hardcover.kp.hardcover_kp')
    ...
    /**
     * get list of hardcover kp
     */
    public function getListOfHardcoverKP(Request $request) {

        $requests = Hardcover::where('type', CreationType::KP)
    ...
    public function downloadRequestExcel(Request $request) {
        ...
        $result = Hardcover::where('type', CreationType::KP)
                ->orderBy('tanggal_submit', 'DESC');
        if(isset($status)) {
           $result = Hardcover::where('type', CreationType::KP)
                    ->where('status', $status)
        ...
        // redirect
        $hardcoverKP = new Hardcover;
        $this->setFlashMessage('danger', $hardcoverKP->messages('emptyExcelRequest'));
        return redirect($hardcoverKP->getRoute(CreationType::KP));
    }

    /**
     * retrieve validated status hard cover kp by api
     */
    public function storeValidateHardcoverKPByAPI() {
        $client = new Client();
        ...
        try {
            ...
            {
                $NPM = $object->NPM;
                // check npm already exist on hardcover kp or not
                // if exist just update it
                // if not insert it
                $hardcover = Hardcover::where('npm', $NPM)
                    ->where('type', CreationType::KP)->first();
                // dd($object);
                if(isset($hardcover)) {
                ...
                $hardcover->type = CreationType::KP;
            ...
            DB::commit();
            Log::info('Completed logic for read validated hardcover kp api ' . now());
        } catch(\Exception $e) {
            DB::rollback();
            Log::error(now(). ' ' . $e);
        }
    ...
    /**
     * retrieve ongoing status hard cover kp by api
     */
    public function storeOngoingHardcoverKPByAPI() {
        $client = new Client();
        ...
        try {
            ...
            DB::commit();
            Log::info('Completed logic for read ongoing hardcover KP api ' . now());
        } catch(\Exception $e) {
            DB::rollback();
            Log::error(now(). ' ' . $e);
        }
