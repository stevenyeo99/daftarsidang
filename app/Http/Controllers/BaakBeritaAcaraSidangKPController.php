...
class BaakBeritaAcaraSidangKPController extends MasterController 
{
    ...
    public function index() {
        ...
        return view('admin.berita_acara.kp.BaakBeritaAcaraKP',
        compact('breadcrumbs', 'title', 'sub_title', 'beritaAcaraMaster_css', 'beritaAcaraKP_css', 'ruanganSidangArr', 'statusBeritaAcaraArr'));
    }

    public function getListBeritaAcaraSidangKPBaak() {
    ...
    function getActionsButton(Acara $beritaAcara) {
        if($beritaAcara->status == 5) {
            ...
            return "<a title='BERI AKSES' href='' class='btn btn-success update-berita-acara-status-confirmation m-t-3' data-type='Kerja Praktek' data-url='{$permission}' data-target='#update-berita-acara-status-confirmation-modal' data-toggle='modal'><span class='fa fa-unlock'></span> Beri Akses</a>";
        }
        return "";
    }
    ...
    public function getRoute($key1, $key2 = null) {
        switch($key1) {
            case 'list':
                return route('baak.berita_acara.kp');
                break;
            case 'update':
                return route('baak.berita_acara.permission_kp', $key2);
                break;
            case 'view':
                return route('baak.view_berita_acara.kp');
                break;
            default: 
                break;
        }
