...
class BaakBeritaAcaraSidangSkripsiController extends MasterController {
    ...
    public function index() {
        ...
        return view('admin.berita_acara.skripsi.BaakBeritaAcaraSkripsi',
        compact('breadcrumbs', 'title', 'sub_title', 'beritaAcaraMaster_css', 'beritaAcaraSkripsi_css', 'ruanganSidangArr', 'statusBeritaAcaraArr'));
    }

    /**
     * get list berita acara sidang skripsi
     */
    public function getListBeritaAcaraSidangSkripsiBaak() {
    ...
    function getActionsButton(Acara $beritaAcara) {
        if($beritaAcara->status == 5) {
            ...
            return "<a title='BERI AKSES' href='' class='btn btn-success update-berita-acara-status-confirmation m-t-3' data-type='Skripsi' data-url='{$permission}' data-target='#update-berita-acara-status-confirmation-modal' data-toggle='modal'><span class='fa fa-unlock'></span> Beri Akses</a>";
        }
        return "";
    }
    ...
    public function getRoute($key1, $key2 = null) {
        switch($key1) {
            case 'list':
                return route('baak.berita_acara.skripsi');
                break;
            case 'update':
                return route('baak.berita_acara.permission_skripsi', $key2);
                break;
            case 'view':
                return route('baak.view_berita_acara.skripsi');
                break;
            default: 
                break;
        }
