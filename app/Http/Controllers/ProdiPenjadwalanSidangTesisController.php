...
class ProdiPenjadwalanSidangTesisController extends MasterController {
    ...
    public function index() {
        ...
        return view("prodi.schedule.tesis", compact('breadcrumbs', 'title', 'sub_title', 'penjadwalan_css', 'jadwalTesis_css'));
    }

    public function getListOfPenjadwalanTesis(Request $request) {
    ...
    public function getRoute($key, $id = null) {
        switch($key) {
            case 'list':
                return route('prodi.penjadwalan.tesis');
            case 'view':
                return route('prodi.penjadwalan.view.tesis', $id);
            case 'jadwal':
                return route('prodi.penjadwalan.assign.tesis', $id);
            default:
                break;
        }
    }
    ...
    public function getPenjadwalanForStoreData(Jadwal $penjadwalan) {
        ...
        return view('prodi.schedule.tesis_schedule', compact('breadcrumbs', 'title', 'sub_title', 'penjadwalan_css', 
        'jadwalTesis_css', 'penjadwalan', 'listOfProdiDDL', 'pembimbingValue', 'isRevision'));
    }
    ...
    public function viewPenjadwalan(Jadwal $penjadwalan) {
        ...
        $backRoute = route('prodi.penjadwalan.tesis');
    ...
    public function notifyProdiAdminToRescheduleIfNeed() {
        ...
        $penjadwalanSidang = $penjadwalanSidangModel::where('status_penjadwalan', 'COMPLETED')
                            ...
                            ->where('sidang_type', CreationType::Tesis)
                            ->get();
        ...
        if($needRevise) {
            ...
            $penjadwalanSidangDistinct = $penjadwalanSidangModel::distinct()->where('sidang_type', CreationType::Tesis)->get(['penjadwalan_by']);
            foreach($penjadwalanSidangDistinct as $object) {
                ...
                try {
                    $tesis = 'Tesis';
                    Mail::to($prodiAdmin)->send(new ReschedulePenjadwalanNotification($prodiName, $tesis));
