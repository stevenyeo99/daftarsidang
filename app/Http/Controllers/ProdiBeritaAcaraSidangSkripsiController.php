...
class ProdiBeritaAcaraSidangSkripsiController extends MasterController  {
    ...
    public function index() {
        $breadcrumbs = $this->getBreadCrumbs('beritaAcaraSidangSkripsi');
        $title = $this->getTitle('beritaAcaraSidangSkripsi');
        $sub_title = $this->getSubTitle('beritaAcaraSidangSkripsi');
        $beritaAcaraMaster_css = 'active';
        $beritaAcaraSkripsi_css = 'active';
        ...
        return view('prodi.berita_acara.skripsi.skripsi', compact('breadcrumbs', 'title', 'sub_title', 'beritaAcaraMaster_css', 'beritaAcaraSkripsi_css', 'ruanganSidangArr'));
    }
    ...
    public function getListOfBeritaAcaraForEachParticipant() {
        ...
        $listOfBeritaAcara = Penguji::join('berita_acara_report AS master_acara', 'master_acara.id', '=', 'berita_acara_participant.berita_acara_report_id')
            ...
            ->where('r.type', CreationType::Skripsi)
    ...
    public function getRoute($key, $id = null) {
        switch($key) {
            case 'list':
                return route('prodi.berita.acara.skripsi');
            case 'form':
                return route('prodi.berita_acara_form.skripsi', $id);
            case 'save':
                return '';
            default:
                break;
    ...
    public function formSidang(Penguji $participant) {
        ...
        $breadcrumbs = $this->getBreadCrumbs('formulirBeritaAcaraSidangSkripsi');
        $title = $this->getTitle('formulirBeritaAcaraSidangSkripsi');
        $sub_title = $this->getSubTitle('formulirBeritaAcaraSidangSkripsi');
        $beritaAcaraMaster_css = 'active';
        $beritaAcaraSkripsi_css = 'active';
        $beritaAcaraMaster = $participant->report()->first();
        $listOfNoteRevision = $participant->revisi()->get();

        return view('prodi.berita_acara.skripsi.skripsi_form', compact('beritaAcaraMaster', 'participant', 'listOfNoteRevision',
         'breadcrumbs', 'title', 'sub_title', 'beritaAcaraMaster_css', 'beritaAcaraSkripsi_css'));
    }
        ...
        try {
            ...
            if($participantType == 1) {
                ...
                if(count($arrayNotes) > 0) {
                    Mail::to($studentEmaillAddress)->send(new NoteRevisiSidangMahasiswa($acara, $beritaAcaraParticipant, $arrayNotes, 'Skripsi'));
                }
                ...
                Mail::to($studentEmaillAddress)->send(new HasilSidangMahasiswa($acara, 'Skripsi', $isLulus));
            } else {
