...
class RequestKPController extends MasterController
{
    /**
    ...
    public function index()
    {
        ...
        $defaultStatusSelection = RequestStatus::Verification;
        $session_statuses = StatusSidang::getStrings();
        $types = CreationType::getStrings();

        return view('admin.request.kp')
                ->with('title', $title)
    ...
    {
        $newCustomRequest = new Student;
        $study_program_id = $newCustomRequest->where('id', $customRequest->student_id)->first()->study_program_id;
        $studyProgramInstance = new StudyProgram;
        $studyProgram = $studyProgramInstance->where('id', $study_program_id)->first();
    ...
    public function downloadRequestExcel(Request $request)
    {
        ...
        if (count($result) > 0) {
            ...
            Excel::create('Laporan Pendaftaran KP', function($excel) use ($result_array) {
    ...
    /**
     * Reject all expired kp requests.
     *
     * @param empty
     *
     * @return empty
    ...
    {
        $breadcrumbs = $this->getBreadCrumbs('changeKpSessionStatus');
        $title = $this->getTitle('changeKpSessionStatus');
        $sub_title = $this->getSubTitle('changeKpSessionStatus');
        $request_css = "active";
        $kp_request_css = "active";
        $btn_label = "Ubah";
        $student = $customRequest->student()->first();
        $session_status = $student->session_statuses()->where('type', CreationType::KP)->first();
    ...
    public function updateSessionStatus(CustomRequest $customRequest, Request $request)
    {
        ...
        $existing_session_status = $student->session_statuses()->where('type', CreationType::KP)->first();
    ...
    public function getRoute($key, $id = null)
    {
        switch ($key) {
            case 'list':
                return route('request.kp');
            case 'view':
                return route('request.kp.view', $id);
            case 'accept':
                return route('request.kp.accept', $id);
            case 'reject':
                return route('request.kp.reject', $id);
            case 'cancel':
                return route('request.kp.cancel', $id);
            // case 'change_session_status':
            //     return route('request.kp.change.session.status', $id);
    ...
    public function getActionsButtons($model, array $extraClassToAdd = [])
    {
        ...
        // enhancement admin baak tidak bisa ubah status 
        if (isset($cancel)) {
            return "<a href='{$view}' title='LIHAT' class='btn btn-info'><span class='fa fa-eye'></span> Lihat </a>
