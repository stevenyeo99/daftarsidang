...
class FinanceUser extends BaseModel implements Authenticatable {
        ...
        switch($key) {
            case 'validation':
                return [
                    ...
                    'email.required' => 'Email user finance tidak boleh kosong.',
            ...
            case 'success':
                switch($keyTwo) {
                    case 'create':
                        return 'Telah membuat user Finance dengan sukses!';
                    case 'update':
            ...
            case 'failDelete':
                return 'User Finance ini tidak bisa dihapus karena telah dipakai ditempat lain.';
            case 'failDeleteSelf':
     ...
     */
    public function rules(FinanceUser $financeUser) {
        if(isset($financeUser->id)) { // when update
            return [
                'username' => 'required|max:50|unique:finance_user,username,'.$financeUser->id,
                'email' => 'required|email|max:50|unique:finance_user,email,'.$financeUser->id,
                'password' => 'required|confirmed',
            ];            
        } else {
            return [
                'username' => 'required|max:50|unique:finance_user',
                'email' => 'required|email|max:50|unique:finance_user',
                'password' => 'required|confirmed',
            ];
        }
    }
