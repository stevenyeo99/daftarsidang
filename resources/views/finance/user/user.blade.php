                ...
                <div class="box-body">
                    <div class="custom-add-new-button">
                        @if(Gate::allows('is-finance-admin'))
                            <a href="{{ route('finance_user.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> User Baru</a>
                        @endif
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        var financeUserDatatable = $('#user_datatable').DataTable({
            'autoWidth': false,
                orderCellsTop: true,
                responsive: true,
                processing: true,
         ...
         $('.search-input-text').on('keyup click', function() {
            ...
            var v = $(this).val();
            financeUserDatatable.columns(i).search(v).draw();
        });
    });
</script>
@endpush